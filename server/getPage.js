...
async function getPage(pageName, hostname) {
  ...
  if (!manifests) {
    const [ssrMetafileString, publicMetafileString, islandsMetafileString] = await Promise.all([
      fs.promises.readFile(path.resolve(ssrDirectory, 'metafile.json'), 'utf-8'),
      fs.promises.readFile(path.resolve(publicDirectory, 'metafile.json'), 'utf-8'),
      fs.promises.readFile(path.resolve(publicDirectory, 'metafile-islands.json'), 'utf-8'),
    ])
    metafiles = {
    ...
    manifests = Object.fromEntries(
      Object.entries(metafiles).map(([key, metafile]) => [
        key,
        Object.entries(metafile.outputs).reduce((acc, [outputFileName, info]) => {
          if (info.entryPoint) {
            acc[info.entryPoint] = {
  ...
  const preloadJs = (metafiles.islands.outputs[islandJSFile]?.imports || [])
    ...
    .map(({ path: filePath }) =>
      path.resolve(publicURLPath, path.relative(publicDirectoryRelative, filePath)),
    )
  ...
  return {
    js: islandJSFile
      ? `${publicURLPath}/${path.relative(publicDirectoryRelative, islandJSFile)}`
      : '',
    preloadJs,
