...
async function getPage(pageName, hostname) {
  ...
  const doesIslandsFileExists = await fs.promises
    .access(islandJSFile, fs.constants.F_OK)
    .then(() => true)
    .catch(() => false)
  ...
  return {
    js: doesIslandsFileExists
      ? `${publicURLPath}/${path.relative(publicDirectory, islandJSFile)}`
      : '',
    preloadJs,
    css: cssFile,
    exports,
