...
- **Import maps and node modules both are needed**: Every import mapped package also needs to be added as a node module so that it can work server side as well. This double addition is not fun. Someone needs to implement a dynamic loader that takes client import map and loads it in on server without having to make devs download everything as a node module.
